...
<template>
  <Portal msg="Hello World" :closePortal="closePortal" v-if="selectedExercise">
  ...
  <section id="workout-card">
    ...
    <div class="workout-grid">
      ...
      <div class="workout-grid-row" v-for="(w, i) in warmup" :key="i">
        <div class="grid-name">
          <p>{{ w.name }}</p>
          <button @click="openPortal(w.name)">
            <i class="fa-regular fa-circle-question"></i>
          </button>
        </div>
        <p>{{ w.sets }}</p>
        <p>{{ w.reps }}</p>
        <input type="text" class="grid-weights" placeholder="10kg" disabled />
      </div>
      ...
      <h6>Reps</h6>
      <h6 class="grid-weights">Weights</h6>
      <div class="workout-grid-row" v-for="(w, i) in workout" :key="i">
        <div class="grid-name">
          <p>{{ w.name }}</p>
          <button @click="openPortal(w.name)">
            <i class="fa-regular fa-circle-question"></i>
